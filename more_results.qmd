...
# Create choropleth maps for each month
months_to_plot <- c("Sep", "Oct", "Nov", "Dec")

for (mon in months_to_plot) {
  month_data <- state_month_avg |>
    filter(month == mon)

  # Merge with map data
  plot_data <- us_states |>
    left_join(month_data, by = "region")

  # Create the plot
  p <- ggplot(plot_data, aes(x = long, y = lat, group = group, fill = avg_depth)) +
    ...
    labs(
      title = paste("Average Depth of Target -", mon, "2023"),
      subtitle = "Outdoor stadiums only | All pass attempts (C/I/IN)",
      caption = paste("States without data shown in grey")
    ) +
    theme_minimal() +
    theme(
      plot.title = element_text(face = "bold", size = 14),
